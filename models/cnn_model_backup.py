...
class CNNModel(nn.Module):
    ...
    def forward(self):
        ...
        save_nii(self.fv_recon_pred.detach().squeeze().cpu().numpy(), 'fv_recon_pred.nii')
        save_nii(self.fp_pred.detach().squeeze().cpu().numpy(), 'fp_pred.nii')
        save_nii(self.fp_pred_replace.detach().squeeze().cpu().numpy(), 'fp_pred_replace.nii')
        save_nii(self.lv_sinogram.detach().squeeze().cpu().numpy(), 'lv_sinogram.nii')
        pdb.set_trace()
